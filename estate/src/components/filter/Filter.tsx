      ...
      >
        ...
        {!isCompact && (
          <div className="flex justify-between items-center mb-4">
            ...
            >
              Properties in <span className="font-bold text-blue-600">{searchParams.get("city") || "All Cities"}</span>
            </motion.h1>
        ...
        >
          ...
          <div className={`relative flex-grow min-w-[180px] ${isCompact ? 'w-full md:w-auto' : ''}`}>
            ...
            <input
              ...
              value={query.city}
              className={`w-full pl-9 pr-4 ${isCompact ? 'py-2 text-sm' : 'py-2.5 md:py-3'} border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`}
            />
          </div>
          ...
          <div className={`hidden md:flex flex-wrap gap-2 ${isCompact ? 'flex-1 justify-between' : ''}`}>
            <select
              ...
              onChange={handleChange}
              value={query.type}
              className={`${isCompact ? 'px-3 py-2 text-xs w-24' : 'px-4 py-3'} border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`}
            >
            ...
            <div className="flex gap-2">
              <input
                ...
                className={`${isCompact ? 'w-20 px-3 py-2 text-xs' : 'w-24 px-4 py-3'} border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`}
              />
              <input
                type="number"
                id="maxPrice"
            ...
            <div className="flex gap-2">
              <motion.button
                ...
                className={`${isCompact ? 'px-3 py-2 text-xs' : 'px-5 py-3'} bg-blue-600 text-white font-medium rounded-lg shadow-md shadow-blue-100`}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                Search
          ...
          {/* Mobile Filter Button for Compact Mode */}
          {isCompact && (
            <motion.button
              onClick={() => setIsFiltersOpen(true)}
              className="md:hidden flex items-center gap-2 bg-blue-50 text-blue-600 px-3 py-2 rounded-lg font-medium text-sm flex-1"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <SlidersHorizontal size={16} />
            ...
            {Object.entries(query).map(([key, value]) => {
              if (value) {
                return (
                  <motion.span 
                    key={key}
                    className={`bg-blue-50 text-blue-600 px-2 py-1 rounded-full ${isCompact ? 'text-xs' : 'text-sm'} flex items-center gap-1`}
